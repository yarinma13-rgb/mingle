  ...
  return (
    <span className="inline-flex max-w-full flex-col items-stretch gap-1">
      <button
        ...
        className={
          className ??
          "mingle-btn-secondary !min-h-10 max-w-full truncate !px-4 !py-2 text-xs"
        }
      >
        {text}
      </button>
...
export function CandidateProfileCvActions({
  ...
  const viewClass = light
    ? "mingle-btn-primary !min-h-10 !px-4 !py-2 text-xs"
    : "mingle-btn-primary !min-h-10 !px-4 !py-2 text-xs shadow-sm";
  const cvClass = light
    ? "mingle-btn-secondary !min-h-10 max-w-[11rem] truncate !px-4 !py-2 text-xs"
